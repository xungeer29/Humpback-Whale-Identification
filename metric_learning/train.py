...
def transform_train(image, label):
    ...
    # view fliplr image as new class whale
    if 0:
        if random.random() < 0.5:
        ...
        image = image[:,:,:3]
    if random.random() < 0.5:
        image = random_angle_rotate(image, angles=(-25, 25))
    # noise
    if random.random() < 0.5:
    ...
    if 1:
        image = random_shift(image, p=0.5)
    if 1:
        image = random_scale(image, p=0.5)
    # todo data augment
    if 0:
        if random.random() < 0.5:
            mask[...] = 0
...
def train(freeze=False, fold_index=0, model_name='seresnext50',min_num_class=10, checkPoint_start=0, lr=3e-4, batch_size=36):
    num_classes = 5004 * 2
    ...
    with open('./data/train_fold_{}.txt'.format(fold_index)) as f_train:
        for line in f_train.readlines():
            name, label = line.strip().split(' ')
            names_train.append(name)
            labels_train.append(label)
    names_valid, labels_valid = [], []
    with open('./data/val_fold_{}.txt'.format(fold_index)) as f_valid:
...
if __name__ == '__main__':
    ...
    model_name = 'seresnext50' #'senet154'
    if step == 1:
        freeze = False
        fold_index = 1
        min_num_class = 10
