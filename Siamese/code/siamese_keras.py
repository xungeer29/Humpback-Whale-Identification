...
def build_model(lr, l2, activation='sigmoid'):
    ...
    print('xa_inp size: {}'.format(xa_inp.shape.as_list()))
    x1 = Lambda(lambda x: x[0] * x[1])([xa_inp, xb_inp])
    x2 = Lambda(lambda x: x[0] + x[1])([xa_inp, xb_inp])
    x3 = Lambda(lambda x: K.abs(x[0] - x[1]))([xa_inp, xb_inp])
    x4 = Lambda(lambda x: K.square(x))(x3)
    print('x1 size: {}'.format(x1.shape.as_list()))
    x = Concatenate()([x1, x2, x3, x4])
    print('concatenate x size: {}'.format(x.shape.as_list()))
    x = Reshape((4, branch_model.output_shape[1], 1), name='reshape1')(x)
    print('reshape01 x: {}'.format(x.shape.as_list()))

    # Per feature NN with shared weight is implemented using CONV2D with appropriate stride.
    x = Conv2D(mid, (4, 1), activation='relu', padding='valid')(x)
    print('conv2d 01 x: {}'.format(x.shape.as_list()))
    x = Reshape((branch_model.output_shape[1], mid, 1))(x)
    print('reshape02 x: {}'.format(x.shape.as_list()))
    x = Conv2D(1, (1, mid), activation='linear', padding='valid')(x)
    print('conv2d 02 x: {}'.format(x.shape.as_list()))
    x = Flatten(name='flatten')(x)
    print('flatten x: {}'.format(x.shape.as_list()))

    # Weighted sum implemented as a Dense layer.
    x = Dense(1, use_bias=True, activation=activation, name='weighted-average')(x)
    print('fc x: {}'.format(x.shape.as_list()))
...
class ScoreGen(Sequence):
    def __init__(self, x, y=None, batch_size=2048, verbose=0):
        ...
        if self.verbose > 0:
            #self.progress = tqdm(len(self), desc='Scores')
            self.progress = tqdm(len(self))
...
def compute_score(verbose=0):
    ...
    """
    featuregen = FeatureGen(train, verbose=verbose)
...
def make_steps(step, ampl):
    ...
    history = model.fit_generator(
        ...
        initial_epoch=steps, epochs=steps + step, max_queue_size=12, workers=6, verbose=0).history
    steps += step
...
if isfile('../models/standard.model'):
