...
if __name__ == '__main__':
    ...
    if args.model == 'ResNet101':
        ...
        model = ResNet101(backbone, num_classes=5004)
    ...
    train_id = []
    for line in lines:
        im, id = line.strip().split(',')
        train_img.append(im)
        train_id.append(id)
    trainpath = '/data2/shentao/DATA/Kaggle/Whale/raw/train/'
    ...
    for imgname in imglist:
        ...
        # predtop1 = output.data.topk(1, dim=1, largest=True, sorted=True) # top5
        ...
        # pred_top1 = predtop1[1].cpu().numpy().flatten()
        # print('top1: {} top5: {}'.format(pred_top1, pred_top5))
        ...
        #print('img: {} top5:{} names: {}'.format(imgname, pred_top5, names))
        #print(imgNames.isin([imgname]))

        
        if train_id[train_img.index(imgname)] in names:
            correct += 1
        n += 1
        print('{}/{} acc@top5: {}'.format(correct, n, float(correct)/float(n)))
